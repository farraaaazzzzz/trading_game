...
# User model — identified by their Google account. `username` holds the
# Google email and is kept as the key threaded through the rest of the app
# (holdings, trade logs, exports) so those didn't need to change.
class User(db.Model):
...
def _safe_username_part(username):
    # Google emails contain characters that are legal in filenames on both
    # Windows and Linux, but "@" reads oddly — swap it for readability only.
    return username.replace("@", "_at_")

def user_log_path(username):
    return os.path.join(LOGS_DIR, f"{_safe_username_part(username)}_trading_log.csv")

@app.cli.command("init-db")
def init_db():
    with app.app_context():
        db.create_all()
        print("Database initialized. Users are created automatically on first Google sign-in.")
...
def google_callback():
    ...
    if not userinfo:
        return jsonify({"error": "Google did not return profile info"}), 400
    ...
    if not user:
        user = User(google_sub=google_sub, username=username, name=name, capital=10000.0)
        db.session.add(user)
        db.session.commit()
...
def export_user_log(username):
    ...
    try:
        ...
        user_log_filename = os.path.join(EXPORTS_DIR, f"{_safe_username_part(username)}_{timestamp}_log.csv")
